...
for (const entry of profileEntries) {
   ...
   if(!profile.groupme) continue
      console.log(`${profile.firstName} ${profile.lastName} (${profile.groupme.info.name}) | ${profile.groupme.info.email} | ${profile.groupme.info.phone_number} | ${profile.defaultGroup} 
